...
def parse_args():
	...
	parser.add_argument('--visualize', type=int, default='0', help='0/1 for visualisation of results')
...
class ShadyRemoval:
	def __init__(self, image_path, visualize):
		...
		self.visualize = (visualize == 1)
		self.convert_lab = False
		self.shadowless = self.process_img(self.img)
	...
	def read_img(self, image_path):
		...
		img = cv.resize(img, (int(img.shape[0]/img_scale_factor), int(img.shape[1]/img_scale_factor)), -1)
		return img
	...
	def create_neighbourhood(self, img, i, j, neighbourhood_size):
		## TODO: padd image!!!
		# extracts neighbourhood of pixels from image
		neighbourhood = np.zeros((neighbourhood_size*2 + 1, neighbourhood_size*2 + 1))
		idx_x = 0
		for x in range((i-neighbourhood_size),(i+neighbourhood_size)):
			idx_y = 0
			for y in range((j-neighbourhood_size),(j+neighbourhood_size-2)):
				if x < len(img) and y < len(img[0]):
					neighbourhood[idx_y,idx_x] = img[x,y]
				idx_y += 1
			idx_x += 1
		return neighbourhood


	def apply_median(self, mask, img):
		neighbourhood_size = 2
		res = img.copy()
		h, w = img.shape
		for i in range(neighbourhood_size, h):
			for j in range(neighbourhood_size, w):
				if mask[i, j] == 255:
					# create neighbourhood for pixel i,j
					neighbourhood = self.create_neighbourhood(img, i, j, neighbourhood_size)
					smoothed = cv.medianBlur(neighbourhood.astype(np.uint8),5)
					# sum smoothed values and normalize with # of nonzero values 
					res[i,j] = np.sum(smoothed.flatten()) / (np.count_nonzero(smoothed.flatten()) + 1)
		return res
	...
	def stretch_img(self, mask, max_v, min_v, g_max, g_min, img):
		...
		# res(i,j) = (fmax-fmin)/(gmax-gmin)) * (img(i,j)-gmin) + mean_v
		res = img.copy()
	...
	def find_min_max(self, arr, percent):
		ct_idx = 0
		for idx,item in enumerate(arr):
			...
			if int(item) == 0:
				...
				ct_idx += 1
				#return idx
			if ct_idx > (percent * len(arr)):
				# excluding the top 10% values
				return idx
	...
	def linear_transform_reconstruction(self, shadow_mask, channel):
		...
		color_mask = self.add_contours(channel, shadow_contours)
		shadow_mask = 255 - mask

		# compute histogram for non-shadow and shadow region
		# apply non-shadow/shadow region as weights
		hist_non_shadow, bin_edges = np.histogram(np.copy(channel), bins=256, range=(0, 256), weights=mask)
		hist_shadow, bin_edges_shadow =  np.histogram(np.copy(channel), bins=256, range=(0, 256), weights=shadow_mask)
		...
		# get highest & lowest val in the highest/lowest 10%
		highest_val = len(hist_non_shadow) - self.find_min_max(np.flip(hist_non_shadow), 0.1)
		lowest_val = self.find_min_max(hist_non_shadow, 0.1)
		
		highest_val2 = len(hist_shadow) - self.find_min_max(np.flip(hist_shadow), 0.02)
		lowest_val2 = self.find_min_max(hist_shadow, 0.02)
		...
		# apply median filtering to reduce edge effect
		smoothed = self.apply_median(shadow_mask, shadowless)

		if self.visualize:
			plt.subplot(121),plt.imshow(shadowless, cmap='gray')
			plt.title('Shadowless channel'), plt.xticks([]), plt.yticks([])
			plt.subplot(122),plt.imshow(smoothed,cmap = 'gray')
			plt.title('Smoothed result'), plt.xticks([]), plt.yticks([])
			plt.show()
		return smoothed


	def lab_shadow_detect(self, img):
		...
		if (a + b) <= 256:
			# shadow
			l[np.where(l <= shadow_thresh)] = max_val
			# non-shadow
			l[np.where(l < max_val)] = min_val
		else:
			# shadow
			l[np.where(l <= shadow_thresh)] = max_val
			b[np.where(b <= shadow_thresh)] = max_val
			# non-shadow
			l[np.where(l < max_val)] = min_val
			b[np.where(b < max_val)] = min_val
	...
	def process_img(self, img):
		...
		if self.convert_lab:
			lab = self.lab_shadow_detect(temp)
			mask_l, grads_l, gx_l, gy_l = self.detect_edges(lab, 0.95, 0.9)

			if self.visualize:
				plt.subplot(121),plt.imshow(lab, cmap='gray')
				plt.title('Adjusted L color channel (LAB)'), plt.xticks([]), plt.yticks([])
				plt.subplot(122),plt.imshow(mask_l,cmap = 'gray')
				plt.title('Shadow edges'), plt.xticks([]), plt.yticks([])
				plt.show()
		...
		res[:,:,1] = res2
		res[:,:,2] = res3
		cv.imshow("rec poisson", res.astype(np.uint8))
		cv.waitKey(0)
		"""
...
if __name__ == '__main__':
	...
	shady = ShadyRemoval(path, args.visualize)
	cv.imwrite('res.'+save_path, shady.shadowless.astype(np.uint8))